...
if __name__ == '__main__':
    ...
    # 加载网络参数
    #select_agent.load_model(actor_path=f'.//Net//select_agent_net//Actor//episode350.pth', critic_path=f'.//Net//select_agent_net//Critic//episode350.pth')
    #agv_agent.load_model(actor_path=f'.//Net//agv_agent_net//Actor//episode350.pth', critic_path=f'.//Net//agv_agent_net//Critic//episode350.pth')
    ...
    for episode in range(num_episodes):
        # with tqdm(total=int(max_steps//2000), desc='Iteration %d' % episode) as pbar:
            ...
            print("========切换到稀疏奖励模式========")
                ...
                # 执行网络硬重置（参考D2SR论文）
                # 只重置名称包含'fc'的层（全连接层）
                #select_agent.partial_reset(layer_patterns=['fc'])  
                #agv_agent.partial_reset(layer_patterns=['fc'])
            ...
            save_fig('AGV1_task_nums', env.agv1.task_nums)
            save_fig('AGV2_task_nums', env.agv2.task_nums)
            save_fig('AGV3_task_nums', env.agv3.task_nums)
            ...
            if (episode+1) % 50 == 0:
                ...
                # 保存模型C:\Users\lenovo\Desktop\AGV\Jsy_project\IPPO_main_heur.py
                ...
                save_fig('5_select_return_list', select_return_list)
                save_fig('5_agv_return_list', agv_return_list)
                #save_fig('TST_finish_goods_list', finish_goods_list)
                #save_fig('TST_order_intime_list', order_intime_list)
                #save_fig('TST_total_task_mean_time', total_task_mean_time)            
                #names = ['select_return_list','agv_return_list', 'finish_goods_list','order_intime_list', 'total_task_mean_time' ]
                save_fig('5_finish_goods_list', finish_goods_list)
                save_fig('5_order_intime_list', order_intime_list)
                save_fig('5_total_task_mean_time', total_task_mean_time)            
                names = ['select_return_list','agv_return_list', 'finish_goods_list','order_intime_list', 'total_task_mean_time' ]
                lists = [select_return_list, agv_return_list, finish_goods_list, order_intime_list, total_task_mean_time]
                output_dir = os.path.join('.', 'results')
                os.makedirs(output_dir, exist_ok=True)
                write_lists_to_csv(lists, names, os.path.join(output_dir, 'data_sparse.csv'))
